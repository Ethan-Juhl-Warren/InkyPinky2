...
Inputs:
- handle: uintptr The open file, as returned by `os.fd`
...
_map_read_only :: proc(handle: uintptr, size: i64) -> (data: []byte, ok: bool) {
	...
	// The view holds its own reference to the file, so the mapping object has done its job as soon as the view exists.
